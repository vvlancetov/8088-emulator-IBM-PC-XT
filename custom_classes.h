...
public:
	...
	uint8 request_IRQ(uint8 irq); //запрос прерывани€ от устройства
	uint8 get_last_int(); //номер последнего прерывани€
	uint16 get_last_int_addr(); //адрес вектора прерывани€
	uint8 next_int();  //прерывание дл€ исполнени€
...
struct dma_cha_param
{
	...
	bool pending = false;
	bool TC_reached = false; //достижение лимита передачи (если нет автопродлени€)
};
...
public:
	...
	void write_port(uint16 port, uint8 data);
	uint8 read_port(uint16 port);
	void sync();		//синхронизаци€
	void ram_refresh(); //обновление пам€ти
...
struct t_counter
{
	...
	bool one_shot_fired = false; //триггер дл€ режима 0
};
...
private:
	...
	std::chrono::steady_clock::time_point timer_start; //дл€ отслеживани€ времени выполнени€
